...
export default function JobsPage() {
  ...
  const fetchLiveJobs = async (cat) => {
    const useCat = cat || categoryRef.current;
    setLoadingLive(true);
    setLiveError(false);
    try {
      const res  = await fetch(`https://remotive.com/api/remote-jobs?category=${useCat}&limit=9`);
  ...
  useEffect(() => {
    fetchLiveJobs();
    const interval = setInterval(() => fetchLiveJobs(), 30 * 60 * 1000);
    ...
    <div style={{ padding:"2rem 0 6rem" }}>
      <div className="wrap">
        ...
        <div style={{ marginBottom:"2.5rem" }}>
          ...
          <p className="section-sub">Top platforms + live remote job listings updated every 30 minutes.</p>
        </div>

        {/* Top Platforms */}
        <div style={{ marginBottom:"3rem" }}>
          <h2 style={{ fontSize:"1.1rem", fontWeight:700, marginBottom:"1rem", color:"var(--cream,#f5f0e8)" }}>🌍 Top Platforms</h2>
        ...
        <div>
          <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", flexWrap:"wrap", gap:"1rem", marginBottom:"1rem" }}>
            <div style={{ display:"flex", alignItems:"center", gap:"0.75rem", flexWrap:"wrap" }}>
              <h2 style={{ fontSize:"1.1rem", fontWeight:700, color:"var(--cream,#f5f0e8)", margin:0 }}>
                🔴 Live Remote Jobs
              </h2>
              <span style={{ fontSize:"0.72rem", fontFamily:"var(--font-mono)", color:"var(--stone,#9ca3af)" }}>
                {lastFetched ? `Updated ${timeAgo(lastFetched)} · auto-refreshes every 30 min` : "Fetching…"}
              </span>
            </div>
          ...
          {loadingLive ? (
            ...
            <div className="card" style={{ textAlign:"center", padding:"2.5rem", color:"var(--muted)" }}>
              <FiBriefcase size={32} style={{ margin:"0 auto 0.75rem", display:"block", opacity:0.4 }}/>
            ...
            <div style={{ display:"grid", gap:"0.75rem" }}>
              {liveJobs.map(job => (
                <div key={job.id} className="card" style={{ display:"flex", gap:"1rem", alignItems:"flex-start", padding:"1rem 1.25rem", flexWrap:"wrap" }}>
                  {job.company_logo ? (
                    <img src={job.company_logo} alt={job.company_name}
                      style={{ width:44, height:44, borderRadius:8, objectFit:"contain", background:"#fff", padding:3, flexShrink:0 }}
                  ...
                  <a href={job.url} target="_blank" rel="noopener" className="btn-gradient"
                    style={{ minHeight:36, padding:"0 0.9rem", fontSize:"0.78rem", display:"inline-flex", alignItems:"center", gap:"0.3rem", flexShrink:0 }}>
                    Apply <FiExternalLink size={12}/>
                  </a>
                </div>
              ))}
