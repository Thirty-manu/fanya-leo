    ...
    <div style={{ padding:"3rem 0 5rem" }}>
      <div className="wrap">
        <div style={{ marginBottom:"3rem" }}>
          <h1 className="section-title">Find freelance jobs today</h1>
        ...
        <div style={{ display:"grid", gridTemplateColumns:"repeat(3,1fr)", gap:"1.25rem" }}>
          {JOBS.map(({name,tag,tc,tb,desc,pay,tags,url})=>(
            <div key={name} className="card" style={{ display:"grid", gap:"0.75rem" }}>
              <span style={{ display:"inline-block", padding:"0.3rem 0.75rem", borderRadius:"var(--radius-full)", fontSize:"var(--text-xs)", fontWeight:700, textTransform:"uppercase", color:tc, background:tb, width:"fit-content" }}>{tag}</span>
              <h3 style={{ fontWeight:700, fontSize:"var(--text-lg)", fontFamily:"var(--font-display)" }}>{name}</h3>
              <p style={{ fontSize:"var(--text-sm)", color:"var(--muted)" }}>{desc}</p>
              <div style={{ display:"flex", flexWrap:"wrap", gap:"0.4rem" }}>
                {tags.map(t=><span key={t} style={{ padding:"0.2rem 0.6rem", borderRadius:"var(--radius-full)", fontSize:"var(--text-xs)", fontWeight:600, background:"var(--surface-3)", color:"var(--muted)", border:"1px solid var(--border)" }}>{t}</span>)}
              </div>
              <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center" }}>
                <span style={{ fontSize:"var(--text-xs)", color:"var(--faint)", fontWeight:600 }}>{pay}</span>
                <a href={url} target="_blank" rel="noopener" className="btn-gradient" style={{ minHeight:36, padding:"0 0.9rem", fontSize:"var(--text-xs)" }}>Apply <FiExternalLink size={12}/></a>
              </div>
            </div>
          ))}
        </div>
